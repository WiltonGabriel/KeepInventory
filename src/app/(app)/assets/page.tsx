...
export default function AssetsPage() {
  ...
  const generalLogCollection = useMemoFirebase(() => (firestore ? collection(firestore, 'log_geral') : null), [
    firestore,
  ]);
 ...
 const generateNewAssetId = async (roomId: string): Promise<string | null> => {
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              ...
              <AlertDialogFooter>
                ...
                <AlertDialogAction onClick={() => handleDelete(row.original)}>Excluir</AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
        </div>
  ...
  return (
    <div>
      ...
      <DataTable columns={columns} data={filteredAssets} emptyStateMessage={isLoading ? 'Carregando...' : 'Nenhum patrimônio encontrado.'} />
    </div>
  );
}
